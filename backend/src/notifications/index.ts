...
export const triggerNotification = async (notification: EventTypes, destination: {
  destination: string,
...
export const triggerUserNotification = async (notification: EventTypes, user: string | User, context: AllContexts) => {
  ...
  for (const webhook of webhooksToTrigger) {
    ...
    await triggerNotification(notification, {
      ...
      type: webhook,
    }, { ...context, receiver_email: userFetched.email, receiver_utorid: userFetched.utorid, receiver_full_name: userFetched.name, frontend_url: process.env.FRONTEND_URL ?? 'http://localhost:3555' });
  }
};
export const triggerMassNotification = async (notification: EventTypes, users: User[] | string[], context: AllContexts) => {
  if (users.length === 0) {
    return;
  }
